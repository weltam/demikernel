...
impl Future for TcpOperation {
    ...
    fn poll(self: Pin<&mut Self>, ctx: &mut Context) -> Poll<()> {
        match self.get_mut() {
            TcpOperation::Accept(..) | TcpOperation::Connect(..) | TcpOperation::Push(..) => {
                Poll::Ready(())
            },
...
impl TcpOperation {
    pub fn expect_result(self) -> (FileDescriptor, OperationResult) {
        match self {
            TcpOperation::Push(fd, Err(e))
            | TcpOperation::Connect(fd, Err(e))
            | TcpOperation::Accept(fd, e) => (fd, OperationResult::Failed(e)),
            TcpOperation::Connect(fd, Ok(())) => (fd, OperationResult::Connect),
            TcpOperation::Push(fd, Ok(())) => (fd, OperationResult::Push),

            TcpOperation::Pop(ResultFuture {
                future,
