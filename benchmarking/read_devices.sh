...
OUTPUT_DIR="/mount/scratch/nvme-reads/"

BENCHMARK_PATH="/home/ashmrtnz/research/nvme/demikernel/benchmarking/build/benchmark"
  ...
  for DEV in ${DEVS[@]}; do
    for QS in ${QSIZE[@]}; do
      for BS in ${BLK_SIZES[@]}; do
        ...
        #   15. bool for picking limit type
        ALL_OPTS=$(python ./gen_cmdline.py ${OP} ${RLAT} ${RPOLL} ${RSUB} \
          ${RBUF}  ${RPCI} ${NUM_REQS} ${DEV} ${RWT} ${QS} ${BS} \
          ${MSIZE} ${FSYNC} ${MSIZE} "false")
        echo "${BENCHMARK_PATH} ${ALL_OPTS}" > ${OP}/${DEV}-${BS}-${RWT}-${QS}.out
        eval "${BENCHMARK_PATH} ${ALL_OPTS} 2>&1 >> ${OP}/${DEV}-${BS}-${RWT}-${QS}.out"
      done
    done
  done
done
